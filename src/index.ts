...
if(canvas){
    ...
    const difficulty:Record<string,{cellUnit:number,tick:number}> =  {
        "easy":{
            cellUnit:20,
            tick:180
        },
    ...
    function changeDifficulty(diffi:string){
    ...
    function move(){
        ...
        if(!gameOver){
            ...
            cxt?.fillRect(snake[0].x,snake[0].y,cellUnit,cellUnit)

            if(cxt)
                cxt.fillStyle = 'green'

            snake.slice(1,).forEach(element => {
                cxt?.fillRect(element.x,element.y,cellUnit,cellUnit)
            });
        }
    }
